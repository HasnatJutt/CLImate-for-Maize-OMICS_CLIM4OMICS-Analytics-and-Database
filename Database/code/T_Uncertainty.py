...
if args.input is not None:
    ...
    if args.output2 is not None:
        ...
        Output_path2 = '../../Database/output/T/uncertainty_plots'
        Output_dir2 = output_fdir(Output_path2)
elif os.path.exists("Database/output/All_Files"):
    Input_dir = "Database/output/All_Files"
    if os.path.exists("APIs/NSRDB/output/NSRDB"):
...
#ax.text (-35, 0.34, '$SD_{G2F-NWS}$' + " = " + str (round (SD_3, 2)), fontsize = 10)
...
plt.title('$SD_{G2F-NSRDB}$' + " = " + str (round (SD_1, 2))+"  ,  "+'$SD_{G2F-DayMet}$' + " = " + str (round (SD_2, 2))+"  ,  "+'$SD_{G2F-NWS}$' + " = " + str (round (SD_3, 2)))
plt.xlabel ("Err-T")
plt.ylabel ("Density")
plt.legend () 
#plt.title ("Error")
