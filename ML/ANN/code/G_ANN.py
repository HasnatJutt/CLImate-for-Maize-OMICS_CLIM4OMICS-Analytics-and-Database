...
if args.input is not None:
    ...
    if os.path.exists(Input_path):
        ...
        if args.output is not None:
            ...
            Output_path = os.path.join(Input_path, '../../../ML/ANN/output/G/pairplots')
...
elif os.path.exists("../../../Database/output/G/more"):
    Input_dir = "../../../Database/output/G/more"
    if args.output is not None:
        Output_dir = output_fdir(args.output)
    else:
        Output_path = '../../../ML/ANN/output/G/pairplots'
        Output_dir = output_fdir(Output_path)
elif os.path.exists("Database/output/G/more"):
    Input_dir = "Database/output/G/more"
    if args.output is not None:
...
# Predict for Test Data    
    predictions = ANN.predict (X_test)
    scatterplot = plt.scatter (y_test, predictions, c = "red",  label = "$R^2$ Min = " + str(round (r2_score (y_test, predictions), 2)))
    RMSE = np.sqrt (metrics.mean_squared_error (y_test, predictions))
...
# Predict Test Data
    predictions = ANN.predict (X_test)
    scatterplot = plt.scatter (y_test, predictions, c = "blue",  label = "$R^2$ Max = " + str(round (r2_score (y_test, predictions), 2)))
    RMSE = np.sqrt (metrics.mean_squared_error (y_test, predictions))
