...
if args.input is not None:
    ...
    if os.path.exists(Input_path):
        ...
        if args.output is not None:
            ...
            Output_path = os.path.join(Input_path, '../../../ML/ANN/output/W/pairplots')
...
elif os.path.exists("../../../Database/output/W/more"):
    Input_dir = "../../../Database/output/W/more"
    if args.output is not None:
        Output_dir = output_fdir(args.output)
    else:
        Output_path = '../../../ML/ANN/output/W/pairplots'
        Output_dir = output_fdir(Output_path)
elif os.path.exists("Database/output/W/more"):
    Input_dir = "Database/output/W/more"
    if args.output is not None:
...
# Feature Selection
    ...
    feature = fs.get_support (indices = True)
    # print(feature)
    selected_feature = list (X.columns [fs.get_support (indices = True)])
...
# # Predict for Test Data    
#         predictions = ANN.predict (X_test)
#         scatterplot = plt.scatter (y_test, predictions, c = "red", label = "$R^2$ Min = " + str(round (r2_score (y_test, predictions), 4)))
#         plt.xlabel ("Observed")
#         plt.ylabel ("Predicted")
#         plt.legend ()
...
#     X_new_min = scaler.transform (X_new_min)
#     y_new_min = ANN.predict (X_new_min)
#     y_new_min = pd.DataFrame (y_new_min, columns = ["Min " + variable])
...
# # Predict for Test Data    
#         predictions = ANN.predict (X_test)
#         scatterplot = plt.scatter (y_test, predictions, c = "blue", label = "$R^2$ Max = " + str(round (r2_score (y_test, predictions), 4)))
#         plt.xlabel ("Observed")
#         plt.ylabel ("Predicted")
#         plt.legend ()
...
#     X_new_max = scaler.transform (X_new_max)
...
# =============================================================================        
# df = pd.DataFrame (list(zip(Experiments_list, RMSE_list, RMSE_list_Min, RMSE_list_Max)), columns = ["Experiment", "RMSE", "RMSE_Min", "RMSE_Max"]) 
...
#             # complete.columns = ["Day", Abb + "M", Abb + "I", Abb + "A"]
#             complete.columns = ["Day", Abb + "M"]
#             complete [Abb + "M_Day"] = Abb + "M" + complete ["Day"].astype (str)
#             # complete [Abb + "I_Day"] = Abb + "I" + complete ["Day"].astype (str)
#             # complete [Abb + "A_Day"] = Abb + "A" + complete ["Day"].astype (str)
#             # complete = complete [["Day", Abb + "I", Abb + "I_Day", Abb + "M", Abb + "M_Day", Abb + "A", Abb + "A_Day"]]
#             complete = complete [["Day", Abb + "M", Abb + "M_Day"]]
            
#             # # specific condition
#             # complete.loc [complete [Abb + "I"] < 0, Abb + "I"] = 0
